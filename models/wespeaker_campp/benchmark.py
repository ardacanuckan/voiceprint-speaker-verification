"""
WeSpeaker CAM++ — Speaker Verification Benchmark
Architecture: CAM++ with Large-Margin fine-tuning
EER: 0.654% | Size: 28MB | Embedding: 512d
"""
...
CACHE = ROOT / "cache"
THRESHOLD = 0.5
MODEL_INFO = {"name": "WeSpeaker CAM++", "eer": "0.654%", "size_mb": 28, "dim": 512}
...
def cmd_enroll(model, wav_path):
    ...
    np.save(str(DATA / "emb_wespeaker_campp.npy"), emb)
    print(f"  Enrolled: {emb.shape} saved")


def cmd_verify(model, wav_path):
    emb_path = DATA / "emb_wespeaker_campp.npy"
...
def main():
    parser = argparse.ArgumentParser(description="WeSpeaker CAM++ Benchmark")
